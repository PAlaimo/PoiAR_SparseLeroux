...
data {
        ...
        int Y[Nobs];       // Vector of cases
        vector[Nobs] lOff; // log-Offset
        
        int<lower=0> N;  // Number of districts
        int<lower=0> Ntimes;  // Number of times
        int tdId[Ntimes, N]; // Associate obs to district

        matrix<lower = 0>[N, N] W; // Adjacency
        int W_n;                         // Number of adjacent region pairs
        
        int<lower = 1> k;    // num of covariates
        matrix[Nobs, k] X;      // Covariate
    }
